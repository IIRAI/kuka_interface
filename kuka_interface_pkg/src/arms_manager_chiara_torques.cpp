...
/*  if(msg.header.frame_id == "right_arm_7_link")
	 R_world2sensor = transform.getBasis().transpose();
  else if(msg.header.frame_id == "left_arm_7_link")
  {
    tf::Matrix3x3 R_off;
    R_off[0][0] = 1;
    R_off[0][1] = 0;
    ...
    R_off[2][2] = 0.8660;
     ...
     R_world2sensor = (transform.getBasis()*R_off.transpose()).transpose();
  ...
  tf::Vector3 meas_t;
    ...
    meas_t = meas_t - bias_torque_left_;
		...
		//Update the flags and stop the arms eventually
            geometry_msgs::WrenchStamped msg_com;
    msg_com.header= msg->header;
